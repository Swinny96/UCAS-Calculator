...
import ucascourses from "./newucas.json";
    ...
    return (
      <SelectForm onSubmit={this.handleSubmit}>
        <Select value={this.state.value} onChange={this.handleChange}>
          ...
          {data.Courses.map((courses, i) => {
            return (
              <Option value={courses} key={i}>
                {courses}
              </Option>
            );
          })}
        ...
        <Select value={this.state.value} onChange={this.handleChange}>
          <Option value="0">Please Select a grade</Option>
          {ucascourses.Courses.map((courses, i) => {
            return (
              <Option value={courses.list.id} key={i}>
                {courses.list.id}
              </Option>
            );
          })}
